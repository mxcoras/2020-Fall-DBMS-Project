...
- 王乐天：环境配置，Git统筹，基础功能实现，代码调试
- 曾蠡：基础功能实现，回收功能实现
- 柒思远：基础功能实现，回收功能实现
- 彭威：正确性测试，YCSB测试，文档框架
...
由于 Ubuntu 18.04 LTS 通过 `apt` 安装的前置依赖版本不满足 `pmdk` 库构建的最低要求，故本项目使用了 Ubuntu 20.04 LTS 虚拟机作为开发环境。
...
以上步骤完成后，可通过 mmap 时传入 `pmem_map_file()` 的参数 `is_pmem` 检验是否模拟成功。经检验，以上步骤在 `Ubuntu 20.04 LTS` 下可以成功模拟 NVM 环境。
...
删除的操作与插入的思路类似。首先用前面提到的方法确定要被删除的元素会存放在哪一张表内，然后遍历该表和它的溢出表进行查找。如果找到了要被删除的元素就记录该元素的位置，然后取出最后一张表的最后一个元素来填充到前面寻找到的元素的位置并使最后一张表的 `fill_num--`，同时检验最后一张表是否为空；如果为空，则将上一张表的 `next_offset` 设为0，最后一个桶的 `pm_flag` 设为0，如果该表为溢出表则将该表释放。返回0代表已经删除了目标元素，如果没有找到要被删除的元素则返回-1。
...
顺序插入100000条数据，并进行查、改、删操作，成功验证正确性。
...
首先将文件夹 benchmark 中的文件名读入，并将属于同个读写比的 load 和 run 文件放在一起执行。在 load 阶段全为插入操作。在 run 阶段每读入一条语句，通过操作符来进行对应操作，记录是否操作失败、操作数、消耗时间。
...
```txt
Task 1
operation with 10w-rw-0-100-load.txt and 10w-rw-0-100-run.txt
start to init
Finished, use time: 63.19200ms
total number: 100000
fail number: 0

start to run 
Finished, use time: 7.40700ms
total number: 10000
insert number: 10000
read number: 0
update number: 0
...
Task 2
operation with 10w-rw-100-0-load.txt and 10w-rw-100-0-run.txt
start to init
Finished, use time: 64.27900ms
total number: 100000
fail number: 0

start to run 
Finished, use time: 2.76400ms
total number: 10000
insert number: 0
read number: 10000
update number: 0
...
Task 3
operation with 10w-rw-25-75-load.txt and 10w-rw-25-75-run.txt
start to init
Finished, use time: 61.11600ms
total number: 100000
fail number: 0

start to run 
Finished, use time: 5.95000ms
total number: 10000
insert number: 7516
read number: 2484
update number: 0
...
Task 4
operation with 10w-rw-50-50-load.txt and 10w-rw-50-50-run.txt
start to init
Finished, use time: 61.40900ms
total number: 100000
fail number: 0

start to run 
Finished, use time: 4.78200ms
total number: 10000
insert number: 5102
read number: 4898
update number: 0
...
Task 5
operation with 10w-rw-75-25-load.txt and 10w-rw-75-25-run.txt
start to init
Finished, use time: 60.87700ms
total number: 100000
fail number: 0

start to run 
Finished, use time: 3.67300ms
total number: 10000
insert number: 2488
read number: 7512
update number: 0
